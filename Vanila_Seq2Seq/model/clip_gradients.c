...
void clip_gradients(struct EncoderRNN *encoder, struct DecoderRNN *decoder, const float max_norm, const int embed_dim)
{
    ...
    total_squared_sum += sum_of_squares(encoder->D_X, encoder->hidden_dim * embed_dim);
    ...
    total_squared_sum += sum_of_squares(decoder->D_X, decoder->hidden_dim * embed_dim);
    ...
    if (global_norm > max_norm) 
    {
        ...
        scale_array(encoder->D_X, encoder->hidden_dim * embed_dim, scale_factor);
        ...
        scale_array(decoder->D_X, decoder->hidden_dim * embed_dim, scale_factor);
